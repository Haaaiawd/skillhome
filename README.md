...
| `skillhome sync --cloud` | Local sync then cloud bidirectional sync (requires remote) |
| `skillhome cloud status` | Show rclone/remote/last-sync state |
| `skillhome cloud remote set <name>` | Set rclone remote (`gdrive:` → `gdrive:skillhome/skills`) |
| `skillhome cloud remote unset` | Clear remote config and disable cloud sync |
| `skillhome cloud pull` | Cloud → local: `rclone bisync` then refresh links |
| `skillhome cloud push` | Local → cloud: converge locally then `rclone bisync` |
| `skillhome cloud sync` | Both: converge → bisync → refresh links |
| `skillhome cloud backups` | List local pre-sync backups (last 3 kept) |
| `skillhome cloud restore <name>` | Restore `skills/` + `config.json` from a backup |
...
Sync the central repo across machines via [`rclone`](https://rclone.org) — works with Google Drive, Dropbox, OneDrive, S3, SFTP, and 40+ other backends. Still no daemon: run `skillhome cloud sync` when you need it.

**One-time setup:**

```bash
# 1. Install rclone (skip if already installed)
sudo apt install rclone        # Debian/Ubuntu
brew install rclone            # macOS
winget install rclone          # Windows

# 2. Create a remote (e.g. Google Drive) — browser OAuth follows
rclone config

# 3. Point SkillHome at it and pull
skillhome cloud remote set gdrive:
skillhome cloud pull
```

`cloud remote set gdrive:` syncs `~/.skillhome/skills/` to `gdrive:skillhome/skills`. To pick your own folder: `skillhome cloud remote set gdrive:my-backup/skills`.

**Daily use:**

```bash
skillhome cloud pull     # fetch remote changes, refresh local links
skillhome cloud push     # converge local skills, upload changes
skillhome cloud sync     # both directions in one step
skillhome cloud status   # rclone / remote / last-sync state
```

- **Only `~/.skillhome/skills/` is synced.** `config.json` (local agent paths), `skillhome.log`, and `backups/` are siblings of `skills/` — they never leave the machine.
- **Automatic backup before every sync.** `cloud pull`/`push`/`sync` first copies `skills/` (including each skill's `.skillhome.json` metadata) and `config.json` to `~/.skillhome/backups/pre-cloud-sync-<YYYYMMDD-HHMMSS>/`. If the backup fails, the sync is aborted — you never sync without a safety net. The last 3 backups are kept; older ones are deleted automatically. `--dry` runs skip the backup since nothing is modified.
- **Recovery:** `skillhome cloud backups` lists all backups with sizes; `skillhome cloud restore <name>` restores `skills/` and `config.json` — and backs up the current state first, so restore itself is reversible.
- **Conflicts keep both sides.** If two machines edit the same file, the newer version keeps the original name and the loser is preserved as `*.conflictN`.
- First sync automatically establishes a baseline (`--resync`, newer side wins). If rclone later demands a resync, run `skillhome cloud pull --resync`.
- New machine bootstrap: `rclone config` → `skillhome cloud remote set gdrive:` → `skillhome cloud pull` → `skillhome init` → `skillhome sync`.
- No rclone or no remote configured? Everything local works exactly as before — cloud commands are purely additive.
...
```json
{
  ...
  "cloudRemote": "gdrive:",        // optional — set via `cloud remote set`
  "cloudEnabled": true,            // optional
  "cloudLastSync": "2026-09-15T10:22:00"  // optional, auto-updated
}
```

Edit `agentDirs` to add a new agent or remove one you don't want managed. The `cloud*` fields are added by `skillhome cloud remote set` and can be ignored if you don't use cloud sync.
