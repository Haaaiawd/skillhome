...
3. **冲突处理** — 出现同名 skill 时：
   ...
   - 相似度 < 95%，**单个**真实副本对中央 → **同名更新**：较新版本写回中央，被替换的版本备份到 `~/.skillhome/backups/`（本地大改后的正常传播路径，不再产生孤儿变体）
   - 相似度 < 95%，同一轮 sync 有**多个**真实副本冲突 → **都保留**，给变体加来源后缀（如 `docx--gemini`）
...
Agent B 有 "docx"（Gemini 的实现）
   ...
   ≥ 95% 相似       < 95% 不同
        ...
        ▼               ├─ 单个真实副本：同名更新，
    合并：保留        较新版本进中央，被替换版本
    较新版本          备份到 ~/.skillhome/backups/
    来源合并          └─ 多个真实副本：都保留，
                        docx（胜者）+ docx--B
```

两个版本都存活——真冲突保留为变体，同名更新的旧版进备份，不丢数据。每个 skill 目录下的 `.skillhome.json` 记录其来源 agent。
